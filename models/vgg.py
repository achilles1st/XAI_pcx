...
def get_vgg(model_fn, ckpt_path=None, pretrained=True, n_class: int = None) -> torch.nn.Module:
    ...
    if ckpt_path:
        checkpoint = torch.load(ckpt_path, map_location=torch.device('cpu'))  # Add map_location argument here
        if "state_dict" in checkpoint:
            checkpoint = checkpoint["state_dict"]
        if "module" in list(checkpoint.keys())[0]:
            checkpoint = {k.replace("module.", ""): v for k, v in checkpoint.items()}
        checkpoint = {k.replace("classifier.last", "classifier.6"): v for k, v in checkpoint.items()}
