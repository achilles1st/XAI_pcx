...
group_class_ids = [
    ...
    [222, 223, 224, 225, 226, 227, 228, 229, 230, 231, 232, 233, 234, 235, 236, 237, 238, 239, 240, 241, 242, 243, 244, 245, 246, 247, 248, 249, 250, 252],
...
groups = dict(
    zip(class_groups, group_class_ids)
)
def get_imagenet(data_path="/hardd/ImageNet-complete", split="train", preprocessing=True):
    if split == "train":
...
class ImageNetDataset(datasets.ImageFolder):
    def __init__(self, root, *args, validate=False, train=True, use_precomputed_labels=False,
        ...
        #     print(
        #         f"``\\textbf{{{name}}}'': [``{'$, ``'.join([self.class_names[x].split(',')[0] for x in ids])}'']".replace("$", "''")
        #     )

        # import json
        # with open('datasets/imagenet_hierarchy.json', 'r') as f:
        #     data = json.load(f)

        self.group_class_ids = group_class_ids
        self.class_groups = class_groups
        self.groups = dict(zip(class_groups, group_class_ids))

        # class_group_ids = []
        #
        # for group in class_groups:
        #     all = []
        #     def get_vals(vals, name):
        #         for val in vals.get("children", []):
        #             if val["name"] == name:
        #                 all.append(val)
        #             # elif name in val["name"]:
        #             #     print(val["name"])
        #             else:
        #                 get_vals(val, name)
        #
        ...
        #             if "index" in child:
        #                 group_class_ids.append(child["index"])
        #             else:
        #                 get_ids(child)
        #     for val in all:
        #         get_ids(val)
        #     class_group_ids.append(np.unique(group_class_ids))
