...
from keras.backend import conv3d_transpose
...
class DenseDeepTaylorDecomposition(layers.Layer):
  ...
  def call(self, inputs, **kwargs):
    R = inputs[0]
    X = inputs[1] + K.epsilon()
    W = self.for_layer.kernel
    
    Wp = K.maximum(W, K.epsilon())
    Zp = K.dot(X, Wp)
    S = R / Zp
...
class Conv3DLayerwiseRelevancePropagationFlat(layers.Layer):
  def __init__(self, for_layer: layers.Conv3D, **kwargs):
    super(Conv3DLayerwiseRelevancePropagationFlat, self).__init__(**kwargs)
    self._auto_track_sub_layers = False
    self.for_layer = for_layer
  ...
  def call(self, inputs, **kwargs):
    R = inputs[0]
    X = K.ones_like(inputs[1])
    W = K.ones_like(self.for_layer.kernel)
  ...
  def get_config(self):
    return super(Conv3DLayerwiseRelevancePropagationFlat, self).get_config()
...
def add_deep_taylor_decomposition_to_model_output(output, class_index=None):
  ...
  while True:
    ...
    elif layer.name == '3dconv_encoding_1':
      current_output = Conv3DLayerwiseRelevancePropagationFlat(layer, name=layer.name + '_lrp_flat')([current_output, layer.input, layer.output])
