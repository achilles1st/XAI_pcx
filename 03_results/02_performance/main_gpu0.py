  ...
  for selected_weight in selected_weights[model_name]:
    ...
    with tf.device('/device:GPU:0'):
      ...
      fill_model_from_hdf5(
        model,
        os.path.join(this_file_path, '../../02_models_training/03_training/weights', model_name, selected_weight['selected'][0]['file']),
        custom_objects={
      ...
      with open(classifications_path + '/classification__boostrap_index-{0:02d}__initial_weights_index-{1:02d}.json'.format(selected_weight['bootstrap_index'], selected_weight['weights_index']), 'w') as outfile:
