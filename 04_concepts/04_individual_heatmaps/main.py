...
data_base_path = '/mnt/neuro/nas2/Work/Graz_plus_R2star/04_concepts/01_heatmaps/deep_taylor_decomposition/relevance/ref_images/R2star_RG/Both_warped'
target_base_path = os.path.join(this_file_path, 'top_ranked_images')
if not os.path.exists(target_base_path):
  os.makedirs(target_base_path)

conceptmap_files = os.listdir(data_base_path)
conceptmap_files.sort(reverse=True)

max_concepts = 4
concepts_count = 0
for channel_index, channel_files in itertools.groupby(conceptmap_files, lambda x: int(x[len('rel_0.351__channel_'):len('rel_0.351__channel_') + 2])):
  if concepts_count >= max_concepts:
    break
  ...
  for subject_index in range(5):
    channel_file = channel_files[subject_index]
    print(channel_file)

    channel_file_path = os.path.join(data_base_path, channel_file)

    # mins
    whole_image = nib.load(channel_file_path)
    ...
    output_filepath = os.path.join(target_base_path, channel_file + '_sum_lt_' + str(min_i) + '_slice_' + str(slice_index) + '_from_histo.png')
    cropped_output_filepath = os.path.join(target_base_path, channel_file + '_sum_lt_' + str(min_i) + '_slice_' + str(slice_index) + '_from_histo_cropped.png')
    convert_append_row.append(cropped_output_filepath)
    ...
    fsleyes_rendering_args = [
      ...
      channel_file_path,
      '--cmap',
      'hot', # 'brain_colours_5redyell_iso',
      '--negativeCmap',
  ...
  convert_append_row.append('+append')
  convert_append_row.append(os.path.join(target_base_path, 'row_' + str(concepts_count) + '.png'))

  # print(' '.join(convert_append_inner))
  subprocess.call(convert_append_row)
  ...
  create_label_args = [
    ...
    'white',
    '-draw',
    f'gravity Center font DejaVu-Sans text 0,0 "concept {concepts_count}, {float(channel_files[0][rel_index:rel_index+5]):.0%}"',
    '-rotate',
    '-90',
  ]
  create_label_args.append(os.path.join(target_base_path, str(concepts_count) + '__channel_index.png'))
  print(' '.join(create_label_args))
  subprocess.call(create_label_args)

  c1_args = [
    'convert',
    os.path.join(target_base_path, str(concepts_count) + '__channel_index.png'),
    os.path.join(target_base_path, 'row_' + str(concepts_count) + '.png'),
    '-background',
    'white',
    '-splice',
    '10x0+0+0',
    ...
    '-chop',
    '10x0+0+0',
    os.path.join(target_base_path, 'row_' + str(concepts_count) + '_with_header.png'),
  ]
  subprocess.call(c1_args)

c_f_args = [
  'convert',
  os.path.join(target_base_path, 'row_1_with_header.png'),
  os.path.join(target_base_path, 'row_2_with_header.png'),
  os.path.join(target_base_path, 'row_3_with_header.png'),
  os.path.join(target_base_path, 'row_4_with_header.png'),
  '-background',
  'white',
  '-splice',
  '0x10+0+0',
